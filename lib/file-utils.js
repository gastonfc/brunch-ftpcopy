...
// Change the base path using the rules provided
// rules = { 'source/': 'destination/' }
exports.moveFile = function(file, rules) {

    if (rules) {
        for (let key of Object.keys(rules)) {
            let src = path.join(key, path.sep);
            let dest = path.join(rules[key], path.sep);

            if (file.indexOf(src) == 0) {
                file = file.replace(src, dest);
                break;
            }
        }
    }
